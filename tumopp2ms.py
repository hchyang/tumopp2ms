...
#########################################################################
# Author: Hechuan Yang
# Created Time: 2018-11-11 18:00:34
# File Name: tumopp2ms.py
# Description: 
#########################################################################
...
def tumopp2tree(tumopp=None,sectors=None,build_tree=None):
    ...
    try:
        ...
        raise StopIteration('You tumopp input file is empty!')
    header=header.rstrip()
    header=header.split()
    assert set(header)==set(['x','y','z','id','ancestor','birth','death','omega']), \
        'The format of your tumopp input file is not correct!'
    for line in input:
        line=line.rstrip()
        columns=line.split()
        info={}
        ...
        node=tree(nodeid=info['id'],
            lens=1,    # the length of each node from tumoppp input is 1 generation 
            x=info['x'],
            y=info['y'],
            z=info['z'],
            birth=info['birth'],
            ...
            omega=info['omega'])
        if node.death!=0:
            node.lifesp=node.death-node.birth
        else:
            node.name=info['id']
            if 'sector' in info:
...
def assign_tipnode_lifesp(nodes=None,shortest=None):
    ...
    '''
    if shortest==None:
        all_lifesp=[node.lifesp for node in nodes.values() if node.lifesp]
        shortest=all_lifesp[0]
        for lifesp in all_lifesp:
            if lifesp<shortest:
                shortest=lifesp
    latest_birth=0
...
def parse_rangef(sectorf=None):
    ...
    with open(sectorf) as input:
        try:
            ...
            raise StopIteration('You sector input file is empty!')
        header=header.rstrip()
        header=header.split()
...
def main():
    ...
    parser.add_argument('tumopp',metavar='INPUT', nargs=1,
        help="population file generated by tumopp, if '-', stdin is used")
    default=None
    parser.add_argument('-x','--xrange',type=check_range,default=default,
        help="the range of x to pick (in the format of 'N,M', N and M are included) [{}]".format(default))
    default=None
    parser.add_argument('-y','--yrange',type=check_range,default=default,
        help="the range of y to pick (in the format of 'N,M', N and M are included) [{}]".format(default))
    default=None
    parser.add_argument('-z','--zrange',type=check_range,default=default,
        help="the range of z to pick (in the format of 'N,M', N and M are included) [{}]".format(default))
    default=None
    parser.add_argument('-r','--rangef',type=str,default=default,
